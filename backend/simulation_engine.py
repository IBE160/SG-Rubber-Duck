...
class SimulationEngine:
    """
    Deterministic, testable simulation runner.
    ...
    def _log_and_broadcast(self, event_type: str, task_id: Optional[int] = None, risk_id: Optional[int] = None, details: Optional[dict] = None):
        details = details or {}
    ...
    def _trigger_risks(self, risks: List[models.Risk], task: TaskState):
        for risk in risks:
            # Risk applies to all tasks if affected_task_ids is empty
    ...
    def run(self):
        ...
        # Calculate baseline CPM to enrich results and events
        ...
        def start_task(tid: int, current_day: int):
            task = state[tid]
